...
//  minimalTunes
//
//  Created by John Moody on 6/10/16.
...
public func demoServer(publicDir: String) -> HttpServer {
    ...
    server["/long"] = { r in
        var libXML = [NSManagedObject]()
        do {
            try libXML = managedContext.executeFetchRequest(NSFetchRequest(entityName: "Track")) as! [NSManagedObject]
        }
        catch {
            print("err")
        }
        let str_version = String(libXML)
        return .OK(.Html(String(libXML)))
    }
